...
void AllTransactionWidget::init()
{
    ...
    QPalette palette;
    palette.setColor(QPalette::Window, QColor(243,245,250));
    setPalette(palette);
...
//    ui->transactionsTableWidget->horizontalHeader()->setFixedHeight(40);
    ...
    setStyleSheet("QPushButton[checkable=\"true\"]{font:12px \"微软雅黑\";background:transparent;border:1px solid rgb(137,129,161);border-radius:9px;color: rgb(137,129,161);}"
...
//    ui->typeWithdrawBtn->setGeometry(ui->typeWithdrawBtn->x(),ui->typeWithdrawBtn->y(),ui->typeWithdrawBtn->width() < 50?50:ui->typeWithdrawBtn->width(),24);
    ...
    ui->typeDepositBtn->hide();
    ui->typeWithdrawBtn->hide();
}
...
//        }
        ...
        {
            QJsonObject assetToIssueObject = operationObject.value("asset_to_issue").toObject();
            AssetInfo assetInfo = HXChain::getInstance()->assetInfoMap.value(assetToIssueObject.value("asset_id").toString());
            unsigned long long issueAmount = jsonValueToULL( assetToIssueObject.value("amount"));
            QString issuer = operationObject.value("issuer").toString();
            QString issueToAccountId = operationObject.value("issue_to_account").toString();

            AccountInfo accountInfo = HXChain::getInstance()->accountInfoMap.value(ui->accountComboBox->currentText());
            if(HXChain::getInstance()->guardAccountIdToName(issueToAccountId) == ui->accountComboBox->currentText() || accountInfo.id == issueToAccountId)
            {
                ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem(ui->accountComboBox->currentText()));
                ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem("+ " + getBigNumberString(issueAmount, assetInfo.precision) + " " + assetInfo.symbol));
                ui->transactionsTableWidget->item(i,3)->setTextColor(QColor(0,170,0));
            }
            else
            {
                QString toAccountName = HXChain::getInstance()->accountIdToName(issueToAccountId);
                if(toAccountName.isEmpty())
                {
                    ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem("id: " + issueToAccountId));
                }
                else
                {
                    ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem(toAccountName));
                }
                ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem(getBigNumberString(issueAmount, assetInfo.precision) + " " + assetInfo.symbol));
                ui->transactionsTableWidget->item(i,3)->setTextColor(QColor(202,135,0));
            }
        ...
        case TRANSACTION_TYPE_PROPOSAL_APPROVE:
        {
            ...
            break;
        case TRANSACTION_TYPE_CREATE_DIRECTOR:
        {
            QString feePayAddress = operationObject.value("fee_pay_address").toString();
            ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("create director")));
        ...
        case TRANSACTION_TYPE_DIRECTOR_LOCK_BALANCE:
        {
            ...
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("director lock balance")));
        }
            break;
...
//            }
        ...
        case TRANSACTION_TYPE_MINE_INCOME:
        {
            ...
            break;
        case TRANSACTION_TYPE_CREATE_ASSET:
        {
            QString assetSymbol = operationObject.take("symbol").toString();

            ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem(tr("create \"%1\"").arg(assetSymbol)));
            ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("create asset")));
        }
            break;
        ...
        case TRANSACTION_TYPE_SIGN_ETH_MULTI_CREATE:
        {
            ...
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("director sign to create %1 multi-sig account").arg(chainType)));
        ...
        case TRANSACTION_TYPE_SIGN_ETH_FINAL:
        {
            ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("director sign ETH trx")));
        ...
        case TRANSACTION_TYPE_SIGN_ETH_COLDHOT_FINAL:
        {
            ui->transactionsTableWidget->setItem(i,2, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,3, new QTableWidgetItem("-"));
            ui->transactionsTableWidget->setItem(i,7, new QTableWidgetItem(tr("director sign ETH cold-hot trx")));
        ...
        case TRANSACTION_TYPE_CITIZEN_PROPOSAL:
        {
            ...
            switch (opType)
            {
            case TRANSACTION_TYPE_CITIZEN_CHANGE_DIRECTOR:
                str += tr("change director");
                break;
            default:
                str += tr("%1 (unkown)").arg(opType);
                break;
