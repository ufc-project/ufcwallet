...
void ProposalPage::showProposals()
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        else if(info.proposalOperationType == TRANSACTION_TYPE_CREATE_DIRECTOR)
        {
            typeStr = tr("create director");
        }
        else if(info.proposalOperationType == TRANSACTION_TYPE_RESIGN_DIRECTOR)
        {
            typeStr = tr("resign director");
        }
        ...
        else if(TRANSACTION_TYPE_FORMAL_DIRECTOR == info.proposalOperationType)
        {
            typeStr = tr("change director");
        }
        else
        {
            typeStr = tr("unknown");
        ...
        if(address.isEmpty())
        {
            ui->proposalTableWidget->setItem(i,4,new QTableWidgetItem(tr("no director")));
        }
...
void ProposalPage::on_proposalTableWidget_cellClicked(int row, int column)
{
    ...
    if(column == 5)
    {
        ...
        if(senators.size() > 1)
        {
            ...
            commonDialog.setText(tr("There are %1 directors in this wallet. Do you want all of them to approve?").arg(senators.size()));
            if(commonDialog.pop())
            {
