...
//        qDebug() << id << result;
    ...
    if( id == "ChangeCrosschainAddressDialog-guard_sign_eths_multi_account_create_trx")
    {
        QString result = HXChain::getInstance()->jsonDataValue(id);
...
void ChangeCrosschainAddressDialog::on_okBtn_clicked()
{
    if(needSenatorSign)
    {
        ...
        {
            CommonDialog commonDialog(CommonDialog::OkOnly);
            commonDialog.setText(tr("You have no signer director in this wallet!"));
            commonDialog.pop();
            return;
        }
        ...
        if(!selectColdKeyFileDialog.filePath.isEmpty() && !selectColdKeyFileDialog.pwd.isEmpty())
        {
            ...
            HXChain::getInstance()->postRPC( "ChangeCrosschainAddressDialog-guard_sign_eths_multi_account_create_trx", toJsonFormat( "guard_sign_eths_multi_account_create_trx",
