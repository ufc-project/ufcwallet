...
#ifdef TEST_WALLET
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(NULL, FALSE,  L"" ASSET_NAME "Wallet_test" );
#else
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(nullptr, FALSE,  L"" ASSET_NAME "Wallet" );
#endif
    //  检查错误代码
...
bool checkOnly()
{
    const char filename[] = "/tmp/FCCWalletlockfile";
