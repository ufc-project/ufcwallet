...
#include "guard/IssueAssetPage.h"
...
Frame::Frame():
    selectWalletPathWidget(nullptr),
    timer(nullptr),
    firstLogin(nullptr),
    normalLogin(nullptr),
    mainPage(nullptr),
    transferPage(nullptr),
    multiSigPage(nullptr),
    minerPage(nullptr),
    bonusPage(nullptr),
    bottomBar(nullptr),
    ...
    newOrImportWalletWidget(nullptr),
    shadowWidget(nullptr),
    poundage(nullptr),
    guardAccountPage(nullptr),
    guardIncomePage(nullptr),
    lockContractPage(nullptr),
    issueAssetPage(nullptr),
    proposalPage(nullptr),
    feedPricePage(nullptr),
    contractTokenPage(nullptr),
    citizenAccountPage(nullptr),
    citizenProposalPage(nullptr),
    timerForAutoRefresh(nullptr),
    functionBar(nullptr),
    contactPage(nullptr)
  /*,
...
//        setLanguage("Simplified Chinese");
    ...
    else
    {
        ...
        if( !dir.exists() || !fileInfo.exists() )  // 如果数据文件被删除了
        {
            ...
            HXChain::getInstance()->configFile->setValue("/settings/feeType", ASSET_NAME);
            HXChain::getInstance()->feeType = ASSET_NAME;
...
//            qDebug() <<  HXChain::getInstance()->middlewarePath << QJsonDocument(object).toJson();
    ...
    trayIcon ->setIcon(QIcon(":/ui/wallet_ui/UFC_logo.ico"));
    //点击托盘执行的事件
...
void Frame::showIssueAssetPage()
{
    emit titleBackVisible(false);

    closeCurrentPage();
    issueAssetPage = new IssueAssetPage(centralWidget);
    issueAssetPage->setAttribute(Qt::WA_DeleteOnClose);
    issueAssetPage->show();
    currentPageNum = 25;
}
...
//        qDebug() << id << result;
    ...
    if( id.startsWith("id-get_asset_imp-"))
    {
        ...
        if(result.startsWith("\"result\":"))
        {
            ...
            if(HXChain::getInstance()->assetInfoMap.contains(assetId))
            {
                ...
                HXChain::getInstance()->assetInfoMap[assetId].currentSupply = jsonValueToULL( dynamicDataObject.value("current_supply"));
            }
        }
    ...
    if( id == "id-list_all_validators")
    {
        QString result = HXChain::getInstance()->jsonDataValue(id);
//        logToFile(QStringList() << result);
        if(result.startsWith("\"result\":"))
        {
            result.prepend("{");
            result.append("}");
            ...
            foreach (QJsonValue v, array)
            {
                ...
                HXChain::getInstance()->postRPC( "id-get_guard_member-" + account,
                                                 toJsonFormat( "get_validator_member", QJsonArray() << account),
                                                 1);
            }
    ...
    if( id.startsWith("id-get_guard_member-"))
    {
        ...
        if(result.startsWith("\"result\":"))
        {
            ...
            if(HXChain::getInstance()->allGuardMap.contains(account))
            {
                QString accountId = object.take("validator_member_account").toString();
                ...
                HXChain::getInstance()->allGuardMap[account].senatorType = object.take("guard_type").toString();
...
//                HXChain::getInstance()->minerMap.insert(account, info);
                ...
                if(!account.isEmpty())
                {
                    HXChain::getInstance()->postRPC( "id-get_miner-" + account,
                                                     toJsonFormat( "get_miner", QJsonArray() << account),
                                                     1);
                }
            }

            HXChain::getInstance()->postRPC( "Finish+get_miner", toJsonFormat( "Finish+get_miner", QJsonArray()), 1);
        }
...
//        qDebug() << id <<result;
    ...
    if(id == "guard-get_proposal_for_voter")
    {
        QString result = HXChain::getInstance()->jsonDataValue(id);
//        qDebug() << id << result;
