...
#ifdef  WIN32
#define NODE_PROC_NAME      "witness_node"WALLET_EXE_SUFFIX".exe"
#define CLIENT_PROC_NAME    "cli_wallet"WALLET_EXE_SUFFIX".exe"
#define COPY_PROC_NAME      "Copy.exe"
#else
#define NODE_PROC_NAME      "./witness_node" WALLET_EXE_SUFFIX
#define CLIENT_PROC_NAME    "./cli_wallet" WALLET_EXE_SUFFIX
...
HXChain::HXChain()
{
    ...
    if( configFile->value("/settings/lockMinutes").toInt() == 0)   // 如果第一次，没有config.ini
    {
        ...
        language = "English";
        configFile->setValue("/settings/feeType", ASSET_NAME);
        feeType = ASSET_NAME;
    ...
    else
    {
        ...
        feeType         = configFile->value("/settings/feeType", ASSET_NAME).toString();
...
void HXChain::getSystemEnvironmentPath()
{
    ...
    foreach(str,environment)
    {
        if (str.startsWith("APPDATA="))
        {
            ...
            walletConfigPath = str.mid(8) + "\\" ASSET_NAME "Data" WALLET_EXE_SUFFIX "_" "1.0.0";
    ...
    foreach(str,environment)
    {
        if (str.startsWith("HOME="))
        {
            ...
            walletConfigPath = str.mid(5) + "/Library/Application Support/"ASSET_NAME"Data" WALLET_EXE_SUFFIX "_" "1.0.0";
            appDataPath = walletConfigPath + "/chaindata";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
    ...
    foreach(str,environment)
    {
        if (str.startsWith("HOME="))
        {
            walletConfigPath = str.mid(5) + "/"ASSET_NAME"Data"WALLET_EXE_SUFFIX;
            appDataPath = walletConfigPath + "/chaindata";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
...
{
    QMap<QString,int> map = { {"BTC",5}, {"ETH",3}, {"HC",1}, {ASSET_NAME,0}, {"LTC",3}, {"ERCPAX",1}, {"ERCELF",0}, {"BCH",3}};
...
{
    QMap<QString,int> map = { {"BTC",4}, {"ETH",3}, {"HC",2}, {ASSET_NAME,2}, {"LTC",3}, {"ERCPAX",2}, {"ERCELF",2}, {"BCH",3}};
...
void HXChain::parseTransaction(QString result)
{
    ...
    case TRANSACTION_TYPE_ISSUE_ASSET:
    {
        ...
        QString toAccountId = operationObject.take("issue_to_account").toString();
        QString toAccountName = accountIdToName(toAccountId);
        if(accountInfoMap.contains(toAccountName))
        {
            transactionDB.addAccountTransactionId(accountInfoMap.value(toAccountName).address, typeId);
        }
    }
        break;
...
{
    postRPC( "id-list_all_validators", toJsonFormat( "list_all_validators", QJsonArray() << "A" << 100));
}
            ...
            foreach (QString account, HXChain::getInstance()->getMyFormalGuards())
            {
                ...
                if(!guardAddresses.contains(accountAddress))
                {
                    if(at.amount.toDouble() <= getAssetAutoWithdrawLimit(at.assetSymbol))
                    {
                        ...
                        logToFile( QStringList() << "guard_sign_crosschain_transaction" << generatedTrxId << account);
                        signedCount++;
            ...
            foreach (QString account, HXChain::getInstance()->getMyFormalGuards())
            {
                ...
                foreach(const GuardMultisigAddress& gma, vector)
                {
                    if(gma.hotAddress == eft.signer)
                    {
                        ...
                        HXChain::getInstance()->postRPC( "autosign-guard_sign_eths_final_trx", toJsonFormat( "guard_sign_eths_final_trx",
                                                                         QJsonArray() << eft.trxId << account));
                        logToFile( QStringList() << "guard_sign_eths_final_trx" << eft.trxId << account);
                        signedCount++;
                        if(signedCount >= 3)    return;
                        break;
                    }
...
void HXChain::fetchProposals()
{
    ...
    postRPC( "guard-get_proposal_for_voter", toJsonFormat( "get_proposal_for_voter", QJsonArray() << HXChain::getInstance()->getPermanentSenators().first()));
...
QString HXChain::accountIdToName(QString accountId)
{
    QString result;
    QStringList keys = accountInfoMap.keys();
    foreach (QString key, keys)
    {
        if(accountInfoMap.value(key).id == accountId)   return key;
    }

    result = guardAccountIdToName(accountId);
    if(!result.isEmpty())   return result;

    result = citizenAccountIdToName(accountId);
    if(!result.isEmpty())   return result;
...
void tableWidgetSetItemZebraColor(QTableWidget *w, int alignment)
{
    for(int row = 0; row < w->rowCount(); row++)
    {
        for(int column = 0; column < w->columnCount(); column++)
        {
            ...
            if(cellWidget)
            {
                QString itemColor = (row % 2) ? "rgb(250,250,250)" : "rgb(255,255,255)";
